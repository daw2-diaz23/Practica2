...
abstract class DataBoundObject {
   ...
   public function Save() {
      if (isset($this->ID)) {
         ...
         foreach ($this->arRelationMap as $key => $value) {
            ...
            if (isset($actualVal)) {
               if (array_key_exists($value, $this->arModifiedRelations)) {
                  $strColumns .= '"' . $key . '", ';
                  $strValues .= ":$value, ";
                  $hasModifiedRelations = true; // Hay relaciones modificadas
               }
            }
         }
         ...
         $strQuery = 'INSERT INTO "' . $this->strTableName . '" (' . $strColumns . ') VALUES (' . $strValues . ')';
   
         if ($hasModifiedRelations) {
            $strColumns = rtrim($strColumns, ', ');
            $strValues = rtrim($strValues, ', ');
   
            $strQuery = 'INSERT INTO "' . $this->strTableName . '" (' . $strColumns . ') VALUES (' . $strValues . ')';
            echo "<br> query... $strQuery <br>";
            unset($objStatement);
            $objStatement = $this->objPDO->prepare($strQuery);
            foreach ($this->arRelationMap as $key => $value) {
               eval('$actualVal = &$this->' . $value . ';');
